    ...
    <div class="relative">
      ...
      <transition leave-active-class="transition ease-in duration-100" leave-from-class="opacity-100"
        ...
        <ListboxOptions
          ...
          <ListboxOption as="template" v-for="language in languages" :key="language.name" :value="language"
...
import type {Language} from "~/interfaces/Invoice";

const languages: { name: Language }[] = [
  'pl' as Language,
].map((lang:Language):{name: Language} => ({name: lang}));
...
const selected = computed<{ name: Language }>({
  get (): { name: Language } {
    return props.modelValue ? {name: props.modelValue} : languages[0]
  },

  set (value: {name: Language}) {
