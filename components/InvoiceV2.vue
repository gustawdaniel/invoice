<template>
  ...
  >
      ...
      <div>
        <div class="relative mr-4 inline-block">
          <div
              ...
              @mouseleave="showTooltip = false" @click="printInvoice()">
        ...
        <div class="relative inline-block">
          <div
              title="Reload Page"
              class="text-gray-500 cursor-pointer w-10 h-10 rounded-full bg-gray-100 hover:bg-gray-300 inline-flex items-center justify-center"
              @mouseenter="showTooltip2 = true" @mouseleave="showTooltip2 = false" @click="reload">
        ...
        <div class="mb-2 md:mb-1 md:flex items-center">
          ...
          <div class="flex-1">
            <input
                ...
                id="inline-full-name" type="text" placeholder="eg. #INV-100001" v-model="invoiceNumber">
          </div>
        </div>
        ...
        <div class="mb-2 md:mb-1 md:flex items-center">
          ...
          <div class="flex-1">
            <input
                ...
                type="date" id="datepicker1" placeholder="eg. 17 Feb, 2020" v-model="invoiceDate" pattern="\d{4}-\d{2}-\d{2}">
          </div>
        </div>
        ...
        <div class="mb-2 md:mb-1 md:flex items-center">
          ...
          <div class="flex-1">
            <input
                ...
                id="datepicker2" type="date" placeholder="eg. 17 Mar, 2020" v-model="invoiceDueDate" pattern="\d{4}-\d{2}-\d{2}">
          </div>
        </div>
      </div>
      <div>
        <div class="w-32 h-32 mb-1 border rounded-lg overflow-hidden relative bg-gray-100">
          <img id="image" class="object-cover w-full h-32" :src="htmlLogo"/>
        ...
        <input name="photo" id="fileInput" accept="image/*" class="hidden" type="file" @change="setLogo">

        <button type="button" class="btn" @click="mongo">Button text</button>
      </div>
    </div>
    ...
    <div class="flex flex-wrap justify-between mb-8">
      <div class="w-full md:w-1/3 mb-2 md:mb-0">
        ...
        <input
            ...
            id="inline-full-name" type="text" placeholder="Billing company name" v-model="billing.name">
        <input
            class="mb-1 bg-gray-200 appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-blue-500"
            id="inline-full-name" type="text" placeholder="Billing company address" v-model="billing.address">
        <input
            class="mb-1 bg-gray-200 appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-blue-500"
            id="inline-full-name" type="text" placeholder="Additional info" v-model="billing.extra">
      </div>
      <div class="w-full md:w-1/3">
        <label class="text-gray-800 block mb-1 font-bold text-sm uppercase tracking-wide">From:</label>
        <input
            class="mb-1 bg-gray-200 appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-blue-500"
            id="inline-full-name" type="text" placeholder="Your company name" v-model="companyName">

        <input
            class="mb-1 bg-gray-200 appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-blue-500"
            id="inline-full-name" type="text" placeholder="Your company address" v-model="companyAddress">

        <input
            class="mb-1 bg-gray-200 appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-blue-500"
            id="inline-full-name" type="text" placeholder="Additional info" v-model="companyInfo">
      </div>
    </div>
    ...
    <div v-for="invoice in items" :key="invoice.id">
      <div class="flex -mx-1 py-2 border-b">
        <div class="flex-1 px-1">
          <p class="text-gray-800" v-text="invoice.name"></p>
        </div>

        <div class="px-1 w-20 text-right">
          <p class="text-gray-800" v-text="invoice.qty"></p>
        </div>

        <div class="px-1 w-32 text-right">
          <p class="text-gray-800" v-text="numberFormat(invoice.rate)"></p>
        </div>

        <div class="px-1 w-32 text-right">
          <p class="text-gray-800" v-text="numberFormat(invoice.total)"></p>
        </div>
    ...
    <div class="py-2 ml-auto mt-5 w-full sm:w-2/4 lg:w-1/4">
      <div class="flex justify-between mb-3">
        ...
        <div class="text-right w-40">
          <div class="text-gray-800 font-medium" v-html="netTotal"></div>
        </div>
      </div>
      <div class="flex justify-between mb-4">
        <div class="text-sm text-gray-600 text-right flex-1">GST(18%) incl. in Total</div>
        <div class="text-right w-40">
          <div class="text-sm text-gray-600" v-html="totalGST"></div>
        </div>
      </div>
      ...
      <div class="py-2 border-t border-b">
        <div class="flex justify-between">
          ...
          <div class="text-right w-40">
            <div class="text-xl text-gray-800 font-bold" v-html="netTotal"></div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div id="js-print-template" ref="printTemplate" class="hidden">
      ...
      <div class="flex justify-between mb-10">
        <div class="w-1/2">
          ...
          <div>
            <div v-text="billing.name"></div>
            <div v-text="billing.address"></div>
            <div v-text="billing.extra"></div>
          </div>
        </div>
        <div class="w-1/2">
          ...
          <div>
            ...
            <div v-text="companyAddress"></div>
            <div v-text="companyInfo"></div>
          </div>
        </div>
      </div>
      ...
      <div class="p-4 max-w-xl mx-auto relative absolute left-0 right-0 overflow-hidden mt-24">
        ...
        <div class="shadow w-full rounded-lg bg-white overflow-hidden w-full block p-8">
          ...
          <div class="mb-4">
            ...
            <input
                class="mb-1 bg-gray-200 appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-blue-500"
                id="inline-full-name" type="text" v-model="item.name">
          </div>
          ...
          <div class="flex">
            <div class="mb-4 w-32 mr-2">
              ...
              <input
                  ...
                  id="inline-full-name" type="text" v-model="item.qty">
            </div>
            ...
            <div class="mb-4 w-32 mr-2">
              ...
              <input
                  class="text-right mb-1 bg-gray-200 appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-blue-500"
                  id="inline-full-name" type="text" v-model="item.rate">
            </div>
...
export default {
  ...
  computed: {
    htmlLogo() {
      return this.logo || `https://placehold.co/300x300/e2e8f0/e2e8f0`;
    },
    logo: {
      get () { return '' },
      // set (value) { this.store.setLogo(value) }
    },
    companyName: {
      get () { return this.$store.name },
      set (value) { this.$store.commit('company/setName', value) }
    },
    companyAddress: {
      get () { return '' },
      // set (value) { this.$store.commit('company/setAddress', value) }
    },
    companyInfo: {
      get () { return '' },
      // set (value) { this.$store.commit('company/setInfo', value) }
    }
  },
  data() {
    return {
      ...
      invoiceDueDate: dayjs().format('YYYY-MM-DD'),
