<template>
  <div>
    ...
    <button class="border px-2 py-1 hover:bg-gray-100" @click="newInvoice">NEW Invoice</button>
    ...
    <table class="min-w-full divide-y divide-gray-300">
      ...
      <tr :key="invoice.id" v-for="(invoice, index) in orderedInvoices">
        ...
        <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{ total(invoice) }}</td>
...
const orderedInvoices = computed<Invoice[]>(() => {
  return invoices.value.sort((a, b) => a.number.split('/').reverse().join('/').localeCompare(b.number.split('/').reverse().join('/')))
})
...
async function clone(invoice: Invoice): Promise<void> {
  ...
  axios.post(config.JSON_URL + '/invoices', inv).then(sync)
}

function remove(id: string): void {
  const yes = confirm('Do you need to remove them?')
  if (yes) {
    axios.delete(config.JSON_URL + '/invoices/' + id).then(sync)
  }
}
